    ...
    def __init__(self):
        ...
        class Notifs:
            ...
            command_usage.add_field(
                ...
                value = textwrap.dedent("""\
                `!twitch notif add [#discord_channel] [streamer_name] [message]` - Добавить уведомление о стриме для стримера в указанный канал
                `!twitch notif remove <#discord_channel> <streamer_name>` - Добавить уведомление о стриме для стримера в указанный канал
                `!twitch notif list [#discord_channel]` - Перечислить список уведомлений о стримах для указанного канала
                `!twitch notif formatting` - Показать переменные, которые вы можете вставить в сообщении уведомления о стриме
                """)
            )
            ...
            prompt1 = "На каком канале вы хотите получать уведомления? Упомяните или введите название одного из них ниже. *(ответ в течение 60 секунд)*"
            prompt2 = "Введите название канала Twitch, для которого вы хотите настроить уведомление. *(ответ в течение 60 секунд)*"
            prompt3 = "ведите пользовательское сообщение, которое вы хотите показывать, когда пользователь выходит в эфир, или введите `default` для сообщения по умолчанию. *(ответ в течение 180 секунд)*"
            ...
            del_success = emoji.cmd_success + "Вы не получите никаких уведомлений в {channel} когда {user} выйдет в эфир."
            add_success = emoji.cmd_success + "Добавлено уведомление для {user} в {channel}"
            list_title = "Уведомление о стриме в **#{channel}**"
            list_embed_limit = "Пользовательские сообщения не были включены в код для вставки, у Discord есть предел, равный 1024 символам. Сообщение все равно покажется, когда пользователь выйдет в эфир."
            ...
            notif_variables.add_field(
                ...
                *`$everyone$`* - Вставлять упоминание @everyone
                *`$here$`* - Вставлять упоминание @here
                """)
            )
        class Permissions:
        ...
        class Streams:
            ...
            command_usage.add_field(
                ...
                `!twitch stream top` - Получить информацию в популярном стриме
                """)
            )
            ...
            **[Смотреть на Twitch](https://twitch.tv/{channel})** или введите `twitch stream watch {channel}`
            ...
            """)
        class Users:
            connections = "Подключения для {user}"
